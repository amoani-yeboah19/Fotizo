 ...
 */
const CHINA_MARQUES = new Set(["BYD", "Changan", "Jetour", "Geely", "Haval", "Avatr", "Landwind"]);
...
export function useChinaSourcing() {
  const listings = useSellerProducts().data ?? [];
  const sales = useSales().data ?? [];
  const vehicles = useStaffQuery(["vehicles"], operationsService.vehicles).data ?? [];

  return useMemo(() => {
    const byMake = new Map<string, typeof vehicles>();
    for (const v of vehicles) byMake.set(v.make, [...(byMake.get(v.make) ?? []), v]);
    const MARQUE_ROWS: MarqueRow[] = [...byMake.entries()]
      .map(([make, models]) => ({
        make,
        units: models.length,
        chineseMarque: CHINA_MARQUES.has(make),
        avgLandedPrice: Math.round(models.reduce((sum, m) => sum + m.landedPrice, 0) / models.length),
        leadTimeWeeks: [
    ...
    const FUEL_MIX = (Object.keys(fuelCounts) as (keyof typeof fuelCounts)[])
      .map((fuel) => ({
        ...
        share: vehicles.length ? Math.round((fuelCounts[fuel] / vehicles.length) * 100) : 0,
      }))
      .filter((f) => f.units > 0)
      .sort((a, b) => b.units - a.units);

    const live = listings.filter((p) => p.status !== "unpublished");
    const needsRestock = (p: (typeof listings)[number]) => p.status !== "unpublished" && p.stock <= RESTOCK_AT;
    const labels = new Map(SHOP_CATEGORIES.map((c) => [c.id, c.label]));
    const byDepartment = new Map<string, typeof listings>();
    for (const p of live) byDepartment.set(p.category, [...(byDepartment.get(p.category) ?? []), p]);
    const DEPARTMENT_ROWS: DepartmentRow[] = [...byDepartment.entries()]
      .map(([id, items]) => ({
        ...
        avgPrice: Math.round(items.reduce((sum, p) => sum + p.price, 0) / items.length),
      }))
      .sort((a, b) => b.skus - a.skus);

    const RESTOCK_QUEUE = listings
      .filter(needsRestock)
      .map((p) => ({ id: p.id, title: p.title, image: p.image, price: p.price, sold: p.sales }))
    ...
    const OUTBOUND_TREND = Array.from({ length: 6 }, (_, i) => {
      ...
      return {
        name: d.toLocaleDateString("en-GB", { month: "short", timeZone: "UTC" }),
        units: sales
    ...
    return {
      ...
      SHIPMENTS,
      UNITS_IN_TRANSIT: SHIPMENTS.filter((s) => s.status === "in_transit" || s.status === "customs").reduce(
        (sum, s) => sum + s.units,
        0,
      ),
      OUTBOUND_TREND,
