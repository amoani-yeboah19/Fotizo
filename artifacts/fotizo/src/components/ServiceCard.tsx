import { Star, CheckCircle2, Clock, Briefcase } from "lucide-react";
...
export function ServiceCard({ service }: { service: Service }) {
  ...
  return (
    <div className="flex flex-col bg-white rounded-2xl border border-border p-6 hover:shadow-lg hover:border-primary/20 transition-all duration-300">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center bg-muted/50 px-2 py-1 rounded-md text-sm font-medium">
          <Star className="w-3.5 h-3.5 fill-accent text-accent mr-1.5" />
          {service.rating} <span className="text-muted-foreground font-normal ml-1">({service.reviewCount})</span>
        </div>
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-border flex items-center justify-between">
        <div>
          <span className="text-lg font-bold text-foreground">${service.hourlyRate}</span>
