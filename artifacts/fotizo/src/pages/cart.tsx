  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container mx-auto px-4 lg:px-8 py-24">
        ...
        {items.length === 0 ? (
          <div className="bg-white rounded-2xl border border-border p-12 text-center max-w-2xl mx-auto mt-12">
                      ...
                      <div className="col-span-6 flex items-center gap-4 w-full">
                        ...
                        <div className="flex-1 min-w-0">
                          <Link href={`/products/${item.productId}`}>
                            <h4 className="font-semibold text-foreground truncate hover:text-primary transition-colors cursor-pointer">{item.title}</h4>
                          </Link>
                          ...
                          <div className="sm:hidden flex items-center justify-between mt-2">
                            ...
                            <button onClick={() => removeItem(item.productId)} className="text-destructive text-sm flex items-center gap-1">
              ...
              <div className="bg-white rounded-2xl border border-border p-6 sticky top-24">
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex justify-between text-muted-foreground">
                    <span>Subtotal</span>
                    <span className="text-foreground font-medium">{format(total)}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>Shipping</span>
                  ...
                  {shipping > 0 && (
                    <div className="text-xs text-accent bg-accent/10 p-2 rounded-lg text-center">
                      Spend {format(50 - total)} more for free shipping!
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between mb-8">
                  ...
                  <span className="text-2xl font-bold text-primary">{format(grandTotal)}</span>
                </div>
