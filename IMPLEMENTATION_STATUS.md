...
Sign-in, wishlist and cart follow-up: the separate /login and /signup pages (which advertised demo credentials that do not exist) were removed; the navbar modal is the only authentication screen, /login and /signup open it after session restoration, and private pages and sign-in prompts open it and return to the original page. Wishlists are stored per account (migration 0006, GET/PUT/DELETE /api/wishlist, idempotent saves, 500-item cap, unpublished products hidden) and drive the product-card and product-page hearts and the buyer dashboard. The marketplace card cart button adds in-stock items; shop items are sourced to order, so zero local stock no longer silently disables the shop "+" (checkout remains disabled). The filter sidebar is sticky beside the results on wider screens. Migration 0006 was applied to the Supabase database. `pnpm run check` passed: 110 tests and all builds.

Shop and marketplace pages: the original /shop and /products designs (as hosted from main) were restored in place of the generic catalogue browser. Both now read 48-item server pages that append as the grid scrolls; shop department, search (debounced) and sort, and marketplace category, price, rating, stock and sort filters run on the server. A best-selling sort (supplier units sold in specs) was added to the API contract. The 681 shop products bundled in the frontend were imported and published under china@fotizo.com with the existing seed script, which also added its 20 shop departments to the categories table. Local /shop shows the same 681 products and layout as the hosted page. The hosted marketplace lists 684 products while this database has 76 marketplace listings, so the hosted site uses a different database. `pnpm run check` passed: 102 tests and all builds.
...
1. Inspect the target schema and take a verified backup. Follow README.md to apply 0001-0006 using the migration runner before this API version starts. Do not use schema push on production.
